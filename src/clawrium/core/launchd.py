...
def label_for(agent_name: str) -> str:
    """Return the launchd Label for the gateway plist of `agent_name`."""
    return f"{LABEL_PREFIX}.{agent_name}"


def plist_path_for(agent_name: str) -> str:
    """Return the absolute path where the gateway plist must live on disk.

    launchd's `bootstrap`/`bootout` ARGV expects this exact path.
    """
    return f"{LAUNCHD_DAEMONS_DIR}/{label_for(agent_name)}.plist"


def render_plist(agent_name: str, template_name: str = "gateway.plist.j2") -> str:
    ...
    return template.render(agent_name=agent_name)


def write_plist(
    client: "paramiko.SSHClient", agent_name: str, contents: str
) -> str:
    """Write `contents` to the gateway plist path on the remote host.
...
def remove_plist(client: "paramiko.SSHClient", agent_name: str) -> None:
    """Remove the gateway plist file from /Library/LaunchDaemons.

...
def test_detail_strips_bidi_and_zero_width_from_body_and_metadata(
    monkeypatch, tmp_path
):
    """Trojan-Source regression test.

    A SKILL.md body and a `_meta.yaml.description` that embed Unicode
    bidi / zero-width / line-separator codepoints must have those
    codepoints removed at the route boundary before reaching the wire.
    Covers every codepoint in ``_CONTROL_STRIP_RE``.
    """
    # Sample codepoints from every covered class. Keep this list in
    # sync with `_CONTROL_STRIP_RE` in `src/clawrium/gui/routes/skills.py`.
    bidi_samples = [
        "؜",   # ALM
        "​",   # ZWSP
        "‌",   # ZWNJ
    ...
    poison = "".join(bidi_samples)
    poison_description = "Normal text" + poison + " end."
    poison_body = (
        "# Header\n"
        "Line with" + poison + " bidi smuggled in.\n"
    )

    catalog = tmp_path / "catalog"
    catalog.mkdir()
    _copy_schemas(catalog)
    skill = catalog / "clawrium" / "trojan"
    skill.mkdir(parents=True)
    # YAML single-quoted preserves the literal bidi codepoints in
    # the parsed string (double-quoted would interpret escapes, which
    # we don't need here since we're writing UTF-8 bytes directly).
    skill.joinpath("_meta.yaml").write_text(
        "name: trojan\n"
        f"description: '{poison_description}'\n"
        "version: 0.1.0\n"
        "compatibility: {openclaw: true, hermes: true, zeroclaw: true}\n",
        encoding="utf-8",
    )
    ...
    result = _run(skills_route.get_skill_route("clawrium", "trojan"))

    # Every covered codepoint must be absent from every string field
    # the API ships. Catches:
    #   - a regression that drops one codepoint from the regex
    #   - a regression that moves _strip_control after byte-truncation
    #   - a regression that forgets to strip metadata or body
    description_out = result["metadata"]["description"]
    body_out = result["body"]
    for codepoint in bidi_samples:
        assert codepoint not in description_out, f"leaked {codepoint!r} in description"
        assert codepoint not in body_out, f"leaked {codepoint!r} in body"
    # The benign content survives.
    assert "Normal text end." in description_out
    assert "bidi smuggled in." in body_out


def test_list_summary_strips_bidi_from_description(monkeypatch, tmp_path):
    """The list endpoint goes through `_summarize`, which has a
    parallel scrub path. Ensure summary descriptions are sanitized."""
    catalog = tmp_path / "catalog"
    catalog.mkdir()
    _copy_schemas(catalog)
    skill = catalog / "clawrium" / "smuggle"
    skill.mkdir(parents=True)
    skill.joinpath("_meta.yaml").write_text(
        "name: smuggle\n"
        "description: \"plain\\u202etset laicifitra plain\"\n"
        "version: 0.1.0\n"
        "compatibility: {openclaw: true, hermes: true, zeroclaw: true}\n"
    )
    skill.joinpath("SKILL.md").write_text("---\nname: smuggle\n---\nbody\n")
    ...
    result = _run(skills_route.list_skills_route())
    entries = result["skills"]["clawrium"]

...
2. **Register the host with a routable address.** If the agent host is reachable over Tailscale or another overlay, add that address and promote it: `clm host address add <host-alias> <overlay-address>` then `clm host address set-primary <host-alias> <overlay-address>`. `clm chat` then dials the overlay address.
...
clm agent memory edit   <agent-name> <file>         # open in $EDITOR, sync back, restart agent if running
